...
class ApiService {
  ...
  async getBranches(): Promise<any[]> {
    ...
    if (result && typeof result === 'object' && Array.isArray(result.branches)) {
  ...
  async getInvoicesAdmin(filters?: {
    ...
    const endpoint = queryString ? `/api/v1/invoices?${queryString}` : '/api/v1/invoices';
    
    const result = await this.request(endpoint);
    // Handle response format: {"invoices": [...], "pagination": {...}}
    if (result && typeof result === 'object' && Array.isArray(result.invoices)) {
  ...
  async submitSignature(invoiceId: string, formData: FormData) {
    ...
    const response = await fetch(`${API_BASE_URL}/api/v1/invoices/${invoiceId}/acknowledge`, {
      method: 'POST',
      headers: {
  ...
  async getInvoiceById(invoiceId: string): Promise<Invoice> {
    ...
    if (result && typeof result === 'object') {
      if ((result as any).invoice && typeof (result as any).invoice === 'object') {
  ...
  async uploadCSV(driverId: string, file: File, routeName?: string, routeDate?: string, selectedRoute?: string): Promise<any> {
  ...
  async previewInvoicePDF(invoiceId: string): Promise<string> {
    const response = await fetch(`${API_BASE_URL}/api/v1/admin/invoices/${invoiceId}/preview-pdf`, {
      method: 'GET',
      headers: {
  ...
  async downloadInvoicePDF(invoiceId: string): Promise<void> {
    const response = await fetch(`${API_BASE_URL}/api/v1/invoices/${invoiceId}/download-pdf`, {
      method: 'GET',
      headers: {
        'Authorization': `Bearer ${this.getToken()}`,
      },
  ...
  async downloadRouteWisePDF(filters: {
    ...
    if (!response.ok) {
      const error = await response.json().catch(() => ({ message: 'Route PDF generation failed' }));
  ...
  async getAvailableRoutes(date: string, driverId?: string): Promise<RouteInfo[]> {
  ...
  async getRoutes(filters?: {
    ...
    const queryString = params.toString();
    const endpoint = queryString ? `/api/v1/routes?${queryString}` : '/api/v1/routes';
    ...
    return { routes: [], pagination: { current_page: 1, total_pages: 1, total_count: 0, per_page: 20 } };
  }
  ...
  async getCustomerVisits(filters?: {
    ...
    const queryString = params.toString();
    const endpoint = queryString ? `/api/v1/customer-visits?${queryString}` : '/api/v1/customer-visits';
    ...
    return { visits: [], pagination: { current_page: 1, total_pages: 1, total_count: 0, per_page: 20 } };
  }

  async getCustomerVisitDetail(customerName: string, routeNumber: number, routeDate: string): Promise<CustomerVisitDetail> {
    const encodedCustomerName = encodeURIComponent(customerName);
    const result = await this.request(`/api/v1/customer-visits/${encodedCustomerName}/${routeNumber}/${routeDate}`);
    return result as CustomerVisitDetail;
  }

  async acknowledgeCustomerVisit(customerName: string, routeNumber: number, routeDate: string, formData: FormData): Promise<any> {
    const encodedCustomerName = encodeURIComponent(customerName);
    const token = this.getToken();
    if (!token) {
      throw new Error('No authentication token found');
    ...
    const response = await fetch(`${API_BASE_URL}/api/v1/customer-visits/${encodedCustomerName}/${routeNumber}/${routeDate}/acknowledge`, {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${token}`,
        // Don't set Content-Type - let browser handle it for FormData
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}));
      throw new Error(errorData.message || 'Failed to acknowledge customer visit');
    }
  ...
  async getGroupedInvoices(filters?: {
    ...
    const queryString = params.toString();
    const endpoint = queryString ? `/api/v1/invoices-grouped?${queryString}` : '/api/v1/invoices-grouped';
    
    const result = await this.request(endpoint);
    if (result && typeof result === 'object' && Array.isArray(result.groups)) {
      return result as GroupedInvoicesResponse;
    }
    return { groups: [], total_groups: 0, pending_groups: 0, delivered_groups: 0 };
  }

  async getCustomerGroupDetail(groupId: string): Promise<CustomerGroupDetail> {
    const encodedGroupId = encodeURIComponent(groupId);
    const result = await this.request(`/api/v1/customer-group/${encodedGroupId}`);
  ...
  async acknowledgeGroup(groupId: string, formData: FormData): Promise<any> {
    ...
    const response = await fetch(`${API_BASE_URL}/api/v1/acknowledge-group/${encodedGroupId}`, {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${token}`,
        // Don't set Content-Type - let browser handle it for FormData
  ...
  async uploadSignature(customerVisitGroup: string, formData: FormData): Promise<any> {
    return this.acknowledgeGroup(customerVisitGroup, formData);
  }

  async updateAdmin(adminId: string, adminData: { name?: string; email?: string; branch_id?: string }): Promise<any> {
    const token = this.getToken();
    if (!token) {
      throw new Error('No authentication token found');
    }
  ...
  async updateDriver(driverId: string, driverData: { driver_name?: string; email?: string; branch_ids?: string[] }): Promise<any> {
    const token = this.getToken();
    if (!token) {
      throw new Error('No authentication token found');
    }
  ...
  async getDriverRoutes(createdDate?: string): Promise<{ routes: Array<{ route_number: number; route_name: string }> }> {
    const params = new URLSearchParams();
    if (createdDate) {
      params.append('created_date', createdDate);
    }
    const queryString = params.toString();
    const endpoint = queryString ? `/api/v1/driver-routes?${queryString}` : '/api/v1/driver-routes';
    
    const result = await this.request(endpoint);
    if (result && typeof result === 'object' && Array.isArray(result.routes)) {
      return result as { routes: Array<{ route_number: number; route_name: string }> };
    }
    return { routes: [] };
  }
}
